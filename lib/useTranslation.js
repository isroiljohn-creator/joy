...
const translations = {
  uz: {
    ...
    "dark_mode": "Tungi",
    // Homepage
    "hero_title": "Uy-joyingizni maskondan toping",
    ...
    "agency_dashboard": "Agentlik paneli"
  },
  ru: {
    // Nav & General
    "home": "Главная",
    ...
    "dark_mode": "Темная",
    // Homepage
    "hero_title": "Найдите своё жилье на maskon",
    ...
    "agencies": "Агентства",
    "agency_dashboard": "Панель агентства"
  },
  en: {
    // Nav & General
    "home": "Home",
    ...
    "dark_mode": "Dark",
    // Homepage
    "hero_title": "Find your home on maskon",
...
export function setLanguage(lang) {
  if (lang !== currentLang) {
    ...
    if (typeof window !== "undefined") {
      localStorage.setItem("maskon-lang", lang);
    }
    listeners.forEach((l) => l(lang));
    // Dispatch custom event for vanilla JS or components outside React tree
    if (typeof window !== "undefined") {
      window.dispatchEvent(new CustomEvent("maskon-lang-change", { detail: lang }));
    }
  }
}
...
export function useTranslation() {
  ...
  useEffect(() => {
    // Read and apply saved language on client side after hydration
    const saved = localStorage.getItem("maskon-lang") || localStorage.getItem("joy-lang") || "uz";
    ...
    window.addEventListener("maskon-lang-change", handleEvent);

    return () => {
      listeners.delete(handleLangChange);
      window.removeEventListener("maskon-lang-change", handleEvent);
    };
  }, []);
