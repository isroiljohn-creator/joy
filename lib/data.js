...
export async function getProfileListings(ownerId) {
  try {
    const { rows } = await pool.query(
      `SELECT l.*, u.name as owner_name, u.is_verified as owner_verified FROM listings l
       LEFT JOIN users u ON l.owner_id = u.id
       WHERE l.owner_id = $1 AND l.deleted_at IS NULL ORDER BY l.id DESC`,
      [ownerId]
    );
    const mapped = rows.map(mapListingFromDb);
    return await attachPriceAnalysis(mapped);
...
export async function getListingCount() {
  try {
    const { rows } = await pool.query("SELECT COUNT(*) FROM listings WHERE status = 'active' AND deleted_at IS NULL");
...
export async function getSimilarListings(listingId, cat, limit = 3) {
  try {
    const { rows } = await pool.query(
      `SELECT l.*, u.name as owner_name, u.is_verified as owner_verified FROM listings l
       LEFT JOIN users u ON l.owner_id = u.id
       WHERE l.cat = $1 AND l.id != $2 AND l.status = 'active' AND l.deleted_at IS NULL
...
export async function getOwnerListingCount(ownerId) {
  try {
    const { rows } = await pool.query(
      "SELECT COUNT(*) FROM listings WHERE owner_id = $1 AND deleted_at IS NULL",
      [ownerId]
    );
    return parseInt(rows[0].count, 10);
  } catch (error) {
...
function mapListingFromDb(row) {
  return {
    ...
    floor: row.floor,
    top: row.top,
    topExpiresAt: row.top_expires_at,
    photo: row.photo,
...
export async function attachPriceAnalysis(listings) {
  ...
  try {
    const { rows: avgRows } = await pool.query(`
      ...
      FROM listings
      WHERE status = 'active' AND deleted_at IS NULL AND area > 0 AND price_num > 0
      GROUP BY cat
    `);
    ...
    return listings.map(l => {
      ...
      const priceNum = parseFloat(l.priceNum) || 0;
      const m2Price = (area > 0 && priceNum > 0) ? (priceNum / area) : 0;
...
export async function adminGetStats() {
  try {
    const totalListings = await pool.query("SELECT COUNT(*) FROM listings WHERE deleted_at IS NULL");
    const activeListings = await pool.query("SELECT COUNT(*) FROM listings WHERE status = 'active' AND deleted_at IS NULL");
    const pendingListings = await pool.query("SELECT COUNT(*) FROM listings WHERE status = 'pending' AND deleted_at IS NULL");
